# 返済ロードマップ

借金返済の管理ツール。収入・支出・返済の記録から**完済予定日を逆算**し、返済戦略ごとの利息と期間を比較します。
データはローカルの **SQLite ファイル**（`saimu.db`）に保存され、外部には一切送信されません。

## 起動

```
cd C:\Users\user\saimu
npm start
```

ブラウザが自動で開きます（開かない場合は http://127.0.0.1:5173/ ）。
停止は `Ctrl+C`。**依存パッケージはありません** — `npm install` は不要です。

| オプション | 用途 |
|---|---|
| `npm start -- --port 5174` | ポートを変える |
| `npm start -- --no-open` | ブラウザを自動で開かない |
| `npm start -- --host tailscale` | Tailscale 経由で開く。外出先からも見られる（→ [スマホから見る](#スマホから見る)） |
| `npm start -- --host 0.0.0.0` | 同じ Wi-Fi の端末からも開く（→ [スマホから見る](#スマホから見る)） |
| `set SAIMU_DB=D:\家計\saimu.db` | DB ファイルの置き場所を変える |
| `set SAIMU_KEY=...` | アクセスキーを自分で決める（既定は `saimu.key` に自動生成） |

## スマホから見る

既定では `127.0.0.1` だけで待ち受けるので、同じ Wi-Fi のスマホからも開けません。
外に出す方法は2つあり、**どちらもアクセスキーが必須**になります。

| | Tailscale 経由 | LAN 直結 |
|---|---|---|
| コマンド | `npm start -- --host tailscale` | `npm start -- --host 0.0.0.0` |
| 外出先から | **見られる** | 見られない |
| 通信の暗号化 | **あり**（WireGuard） | なし（平文の HTTP） |
| 同じ Wi-Fi の他人から | **届かない** | 届く（鍵で弾く） |
| ファイアウォール | 基本は不要 | 手動で開ける必要あり |
| 必要なもの | PC とスマホに Tailscale | なし |

**外出先でも使うなら Tailscale をおすすめします。** LAN 直結は自宅 Wi-Fi 限定で。

### Tailscale 経由（おすすめ）

```
# 1. PC とスマホの両方に Tailscale を入れて、同じアカウントでログインする
#    https://tailscale.com/download

# 2. Tailscale のアドレスだけで待ち受ける（LAN には出ない）
npm start -- --host tailscale
```

`--host tailscale` は起動時に `100.64.0.0/10`（Tailscale が使う CGNAT 範囲）のアドレスを探して、
**そこだけ**で待ち受けます。`0.0.0.0` と違って同じ Wi-Fi の他人からは届きません。
Tailscale が動いていなければ、その場で理由を出して止まります。

表示された URL をスマホで一度開けば完了です。MagicDNS を有効にしていれば
`http://<PCのホスト名>:5173/` でも開けます。

### LAN 直結

```
# 1. ファイアウォールを開ける（管理者の PowerShell で一度だけ。LAN 内からのみ許可）
New-NetFirewallRule -DisplayName "saimu 5173 (LAN only)" -Direction Inbound -Action Allow `
  -Protocol TCP -LocalPort 5173 -RemoteAddress 192.168.2.0/24 -Profile Any

# 2. LAN に出して起動する
npm start -- --host 0.0.0.0
```

### GitHub Pages 版（Google スプレッドシート）

PC を起動しておかなくても、スマホから閲覧も記録もできる版です。
画面のコードは `public/index.html` ただ1つで、`npm run build:web` がデータ層だけを
Google スプレッドシート版に差し替えて `docs/index.html` を作ります。UI は二重管理しません。

```
npm run build:web     → docs/index.html
```

**ログインした人が、その人自身のデータだけを見ます。** スコープは `drive.file` だけなので、
このアプリが触れるのは「このアプリが作ったファイル」に限られます。他人のドライブは見えませんし、
あなたのシートも他人には見えません。役割分担の設定は要りません。

#### 1. Google Cloud の設定（一度だけ）

1. [Google Cloud Console](https://console.cloud.google.com/) でプロジェクトを作る
2. **API とサービス → ライブラリ** で次の2つを有効にする
   - Google Sheets API
   - Google Drive API
3. **OAuth 同意画面** → 外部 → アプリ名とメールを入れる
   - スコープに `.../auth/drive.file` を追加する
   - **テストユーザー**に自分のメールアドレスを追加する
4. **認証情報 → 認証情報を作成 → OAuth クライアント ID**
   - 種類：**ウェブ アプリケーション**
   - **承認済みの JavaScript 生成元** に `https://rbaasemo9a-cell.github.io` を追加
     （末尾にスラッシュやパスを付けないこと。生成元はホスト名までです）
   - リダイレクト URI は不要です（トークン方式のため）
5. 表示された**クライアント ID** を控える

| 同意画面の設定 | ログインできる人 | 各自が見るもの | Google の審査 |
|---|---|---|---|
| テスト（既定） | 登録した人だけ（最大100） | 各自のシートだけ | 不要 |
| 本番公開 | 誰でも | 各自のシートだけ | **不要**（`drive.file` は非機密スコープのため） |

自分だけで使うならテストのままで十分です。テストモードでは定期的な再ログインが要る場合があります。

#### 2. GitHub Pages に置く

リポジトリの初期化と初回コミットは済んでいます（リモートも設定済み）。
GitHub 側に `saimu` リポジトリを作ってから、**新しいターミナルで**:

```
git push -u origin main
```

初回はブラウザが開いて GitHub のログインを求められます（Git Credential Manager）。

> **無料プランでは、Pages を公開できるのは public リポジトリだけです。**
> private リポジトリからの公開には GitHub Pro が要ります。
> public にしてもデータは入りません（`saimu.db` も `saimu.key` も `.gitignore` 済み）。
> クライアント ID もコードに埋め込まず、各端末の localStorage に置きます。

push できたら **Settings → Pages** で、ソースを `main` ブランチの **`/docs`** フォルダに。
数分後に `https://rbaasemo9a-cell.github.io/saimu/` が開きます。

以後、画面を直したら `npm run build:web` してから commit / push します。

初回だけクライアント ID の入力を求められます。貼り付けると**この端末の localStorage にだけ**保存され、
どこにも送信されません。次に「Google でログイン」を押すと、あなたのドライブに
`返済ロードマップ` というスプレッドシートが作られます。

> **GitHub Pages は必ず公開されます。** private リポジトリでもサイト自体は誰でも開けます。
> ただし開いても見えるのは**ログイン画面だけ**で、データはあなたの Google アカウントの中にあります。

#### 3. 今のデータを移す

ローカル版の「データ」画面で **JSON に書き出す** → Pages 版の「データ」画面で
**JSON バックアップを読み込む**。日割り計算の規則は両方とも同じなので、数字は変わりません。

#### ローカル版との違い

| | ローカル版（`npm start`） | GitHub Pages 版 |
|---|---|---|
| 保存先 | 手元の `saimu.db` | あなたのドライブのスプレッドシート |
| PC の起動 | 必要 | **不要** |
| 書き込みの安全性 | SQLite のトランザクション | 全シートを1回の `batchUpdate` で置換 |
| 同時編集 | — | 別端末が先に書いていれば弾く（`meta` の版番号で検出） |
| 外部への送信 | なし | Google のみ |

**2つは別々の保存先です。** 併用すると内容がずれるので、どちらかに寄せてください。

### アクセスキー

どちらの方法でも、起動するとスマホ用の URL が表示されます。
**一度だけ**これを開けば、以後は鍵なしの URL で開けます。

```
  [Tailscale] 外出先からも開けます。通信は暗号化されています。
    http://100.101.102.103:5173/?k=k7m4p2xq9a
    MagicDNS を使っているなら  http://desktop-jort9h7:5173/?k=k7m4p2xq9a
```

| 仕組み | 中身 |
|---|---|
| 鍵の保存先 | `saimu.key`（git 管理外。消すと次回に作り直します） |
| 鍵の受け渡し | `?k=` で一度渡すと Cookie（1年・HttpOnly・SameSite=Lax）に移り、URL からは消えます |
| 同じ PC から | `127.0.0.1` からの接続は鍵なしで通ります。PC での使い勝手は変わりません |
| 鍵が違うとき | 画面もAPIも 401。失敗が10回続いた IP は10分間 429 で締め出します |
| API から使う | `X-Saimu-Key: <鍵>` ヘッダでも通ります |
| 鍵を変える | `saimu.key` を消して再起動、または `SAIMU_KEY` を指定 |

#### ログインの回数について（GitHub Pages 版）

Google のアクセストークンは**1時間で切れます**。この長さは Google 側で決まっていて、
アプリからは変えられません（`expires_in` の値をそのまま使っています）。

延長するには**更新鍵（リフレッシュトークン）**が要りますが、それを受け取るには
クライアントシークレットを持てる場所＝サーバーが必要です。GitHub Pages のような
静的なサイトにシークレットは置けないため、ブラウザだけで動く限りこの制約は外せません。

そこで、**読み込んだ内容をこの端末に控えておき、開いたらまずそれを表示**します。
残高を見るだけならログインは要りません。控えを見せている間は下に帯が出ます。

```
この端末に保存された 8月23日 10:30 時点の内容です   [最新にする]
```

記録を追加・変更するときと、「最新にする」を押したときだけログインが要ります。
**控えがある間は、こちらから勝手に認証しにいきません。** 自動で取り直そうとすると
Google のアカウント選択が出てしまい、静かに終わる保証が無いためです（特にスマホ）。

使っている最中は、期限切れの10分前から先回りして取り直します。
許可済みでセッションも新しいので、たいていは何も出ずに通ります。

ログイン画面には**なぜ要るのか**（期限切れ／記録なし／取り消された）が表示されます。

控えとアクセストークンは、この端末のこのサイト専用の領域に置かれます。
ログアウトすると両方消えます。

Tailscale の場合、鍵は**二重の守り**です（Tailscale 自体が端末を認証し通信を暗号化しているため）。
LAN 直結の場合は、鍵が唯一の守りになります。

`--host` を付けなければ従来どおり `127.0.0.1` のみで、鍵の仕組みは一切動きません。

## コマンド

| コマンド | 内容 |
|---|---|
| `npm start` | サーバーを起動する |
| `npm test` | 下の全部（合計290件） |
| `npm run build:web` | GitHub Pages 版 `docs/index.html` を組み立てる |
| `npm run test:ui` | 画面操作をヘッドレス Edge で確認する（16件） |
| `npm run test:auth` | アドレス判定と、外部公開時のアクセスキー（30件） |
| `npm run test:mobile` | スマホ幅で崩れていないかを実測する（19件） |
| `npm run test:web` | GitHub Pages 版の組み立てとデータ層（40件） |
| `npm run lint` | JS 構文・HTML 構造・テーマトークンの静的チェック |
| `npm run backup` | DB ファイルの複製を `backups/` に作る |
| `npm run schema` | 現在のスキーマと件数を表示する |

## 画面

| 画面 | できること |
|---|---|
| ダッシュボード | 完済予定日、残高の推移、進捗率、今月の収支と発生利息 |
| 借入・返済 | 借入先ごとの元金／未払利息／年利／最低返済額の管理、返済と追加借入の記録 |
| 収入・支出 | 収入と支出をカテゴリ別に記録、月次集計、カード払いの引落月と取り置きの警告、支出の経費計上 |
| 目標 | 完済目標日から必要な月額を逆算。毎月の固定収支から「あといくら稼ぐ／節約するか」を出す |
| ふるさと納税 | 収入・経費・控除から、自己負担2,000円で収まる寄付額の上限を計算 |
| シミュレーション | 雪だるま式で完済時期を試算。毎月50万円まで動かせる |
| データ | DB の状態、バックアップ、JSON の書き出し・取り込み |

## 構成

```
saimu/
├─ package.json        依存なし。Node 22.5+ 同梱の node:sqlite を使う
├─ server.js           HTTP サーバー・アクセスキー・JSON API
├─ netinfo.js          LAN と Tailscale のアドレス判定（副作用なし）
├─ db.js               スキーマ・問い合わせ・トランザクション
├─ public/
│  └─ index.html       UI 一式（CSS と JS を含む単一ファイル）＝画面の唯一の出どころ
├─ web/                GitHub Pages 版のデータ層（Google スプレッドシート）
│  ├─ store.js         ログイン・シートの読み書き
│  ├─ api.js           server.js と同じ形の API をブラウザで再現
│  ├─ boot.js          ログイン画面と起動手順
│  └─ gate.css         ログイン画面の見た目
├─ docs/               ← npm run build:web が生成（GitHub Pages の公開元）
├─ tools/
│  ├─ test.js          データ層とシミュレーションの検証
│  ├─ uitest.js        ヘッドレス Edge で画面操作を検証
│  ├─ authtest.js      LAN 公開時のアクセスキーを検証
│  ├─ mobiletest.js    スマホ幅での崩れを実測して検証
│  ├─ build-web.js     docs/index.html を組み立てる
│  ├─ webtest.js       組み立て結果とデータ層を検証
│  ├─ lint.js          静的チェック
│  ├─ backup.js        npm run backup
│  ├─ schema.js        npm run schema
│  └─ clear-txns.js    収入・支出だけを空にする
├─ saimu.db            ← 起動時に自動生成（git 管理外）
├─ saimu.key           ← LAN 公開時に自動生成（git 管理外）
└─ backups/            ← バックアップ先（git 管理外）
```

### データ設計

```
debts       借入先・元金・未払利息・利息計算の起算日・当初借入額・年利・最低返済額
            origin_* は「この日時点でこの残高だった」という起点。返済では動かさない
borrows     追加で借りた記録。返済と同じ時間軸に並べ、日付順に再生して残高を出す
cards       使っているクレジットカード
card_bills  カードごと・月ごとの請求額。登録した月はこれが引落の正になる
txns        収入と支出（type で区別）。pay_month は引落月、card_id はどのカードか
            cost_rate は経費に計上する割合（0〜100）。家事按分もこれで表す
repayments  返済の記録。debt_id は debts を参照し ON DELETE CASCADE
fixed_items 毎月決まって出入りするお金。項目名・毎月の日付・金額（計画用。収支の記録とは別）
tax_years   ふるさと納税の上限を出すための、年ごとの申告内容。0 は未入力で、記録から集計される
goals       完済目標日・月々の返済目標・生活防衛資金（1行のみ）
```

借入の残高は **元金 (`principal`) と未払利息 (`interest_accrued`) に分けて** 持ちます。
最初に登録するのはこの2つと**起算日 (`accrued_at`)** で、以降の利息は起算日からの経過日数で自動計算されます。

DB に入っているのは「起算日時点」の値です。`GET /api/state` は起算日から今日までの分を足した
`interestToday`（今日時点の未払利息）と `balance`（元金＋未払利息）も一緒に返します。
足した結果を書き戻すのは**返済を記録したときだけ**なので、画面を開いただけで DB が書き変わることはありません。

**返済の記録と残高の更新は一つのトランザクションにまとめています。**
「記録は残っているのに残高が減っていない」という中途半端な状態にはなりません。取り消しも同様です。

旧バージョン（`balance` 一本）の DB は、起動時に自動で移行します。残高はすべて元金として引き継ぎ、
未払利息は 0、起算日は移行した日になります。過去に遡って利息を作り出すことはしません。
`balance` しか入っていない古い JSON バックアップもそのまま読み込めます。

`PRAGMA journal_mode = WAL` を有効にしているため、`saimu.db-wal` と `saimu.db-shm` が並んでできます。これらも DB の一部です。手でコピーしてバックアップを取る場合は3つまとめて扱ってください。`npm run backup`（`VACUUM INTO`）なら1ファイルに畳まれるので、そちらが確実です。

### API

既定では `127.0.0.1` のみ。`--host` で外に出したときは、ループバック以外からの接続に
アクセスキー（Cookie か `X-Saimu-Key` ヘッダ）が要ります。
書き込み系は**更新後の全状態を返す**ので、画面と DB がずれません。

```
GET    /api/state          全データ
GET    /api/stats          DB のパス・容量・件数
POST   /api/debts          借入を追加        PUT/DELETE /api/debts/:id
POST   /api/repayments     返済を記録        DELETE /api/repayments/:id
POST   /api/borrows        追加借入を記録    DELETE /api/borrows/:id
POST   /api/txns           収支を追加        DELETE /api/txns/:id
POST   /api/cards          カードを追加      PUT/DELETE /api/cards/:id
POST   /api/cardbills      請求額を登録      DELETE /api/cardbills/:id
POST   /api/fixed          固定収支を追加    PUT/DELETE /api/fixed/:id
POST   /api/tax            年の申告内容を保存  DELETE /api/tax/:year
POST   /api/txncost        カテゴリをまとめて経費に  PUT /api/txncost/:id で1件
PUT    /api/goals          目標を保存
POST   /api/backup         VACUUM INTO でDB複製
GET    /api/export         JSON書き出し      POST /api/import で取り込み
POST   /api/sample         サンプルデータ    POST /api/wipe で全消去
```

## SQL で直接見る

DB は普通の SQLite ファイルなので、任意のツールで開けます。

```sh
# 借入先ごとの返済総額と利息総額
SELECT d.name,
       SUM(r.amount)    AS 返済総額,
       SUM(r.interest)  AS うち利息
FROM repayments r JOIN debts d ON d.id = r.debt_id
GROUP BY d.id ORDER BY 2 DESC;

# 現在の残高の内訳と、これから1日あたりに付く利息
SELECT name,
       principal                        AS 元金,
       interest_accrued                 AS 未払利息,
       accrued_at                       AS 起算日,
       ROUND(principal * rate / 100 / 365, 2) AS 日あたり利息
FROM debts ORDER BY 元金 DESC;

# 月ごとの収支
SELECT substr(date,1,7) AS 月,
       SUM(CASE WHEN type='income'  THEN amount ELSE 0 END) AS 収入,
       SUM(CASE WHEN type='expense' THEN amount ELSE 0 END) AS 支出
FROM txns GROUP BY 1 ORDER BY 1 DESC;
```

## 現金回収の仕事（Uber Eats など）をしている場合

お客さんから現金を受け取り、あとで運営会社にカードで返す働き方では、
**受け取った現金の一部は自分のものではありません**。そのまま収入として扱うと、
返済に回せる余力が実態より大きく見えます。

記録のしかたは2手順です。

| いつ | 何を | 区分 |
|---|---|---|
| 現金を受け取ったとき | 受け取った全額 | 収入 →「配達（現金回収）」 |
| 運営会社に返したとき | 返した額 | 支出 →「預かり金の返却」、引落は「来月」、カードを選ぶ |

両者が打ち消し合うので、**期間を通した合計は実際の報酬と一致します**。

```
現金回収  +30,000     8月に受け取る
Uber返却  −18,000     カード払い → 9月に口座から出る
────────────────────
差引      +12,000  ← 実際の配達報酬
```

### カードごとの請求額

カードを登録すると、**月ごとの請求額**を入れられます。カード会社の明細どおりの額を入れれば、
一件ずつ記録しなくても取り置きが正確になります。

```
カードの引落                  2026-09 に口座から出る分
  楽天カード      ¥48,200    請求額を登録済み
  三井住友VISA     ¥6,800    明細から推定
```

**請求額を登録した月は、その額が正になります。** 同じカードの明細を入れていても、
そちらは内訳として扱い、二重には数えません。登録が無い月だけ、明細の合計で代用します。
0円を登録すると「その月は引落なし」になります。

| 状態 | その月の引落に使われる額 |
|---|---|
| 請求額を登録済み | **登録した額**（明細は内訳扱い） |
| 未登録・明細あり | 明細の合計（推定） |
| 未登録・明細なし | 0円 |

カードを削除すると請求額も消えますが、**明細は残り**、現金・口座払いとして数えられます。

### 取り置きの警告

問題はタイミングです。現金は今すぐ手に入りますが、カードの引落は翌月です。
そのままだと回収した月が実態より裕福に見え、**手元の現金を使い切って翌月払えなくなります**。

そこで、翌月に確定している引落を「取り置きが必要な額」として差し引き、
**今月まだ使ってよい額**を収入・支出画面とダッシュボードに出します。
足りない場合は不足額を赤で警告します。

```
残り（余力）   ¥-25,000
  取り置き後   ¥-61,000
  → 来月の引落 ¥36,000 が確定。今月まだ使ってよいのは ¥0。
    このままだと来月の引落に ¥61,000 足りません。
```

この取り置きは、カード払いにした支出すべてが対象です（現金回収の仕事に限りません）。

## 目標からの逆算（いくら稼げばいいか）

目標画面に、**毎月決まって入るお金と出るお金**を登録できます。
**項目名・毎月の日付・金額**の3つで登録し、日付の早い順に並びます（日付は省略可）。
そこから、返済目標に届くまで**あといくら必要か**を出します。

```
収入  25日  給与（本業）      ¥280,000
支出   4日  楽天モバイル        ¥3,278
支出  27日  家賃              ¥82,000
支出  31日  国民健康保険       ¥45,000
支出    —  食費              ¥50,000
```

毎月の返済も固定で出ていくお金として一緒に引きます。

```
固定の収入   ¥280,000
固定の支出   ¥180,278   生活にかかる分
毎月の返済   ¥120,000   返済目標（最低返済 7.7万 込み）
残り        ¥-20,278   収入 − 支出 − 返済

毎月 ¥120,000 を返すには ¥20,278 足りません。

  副業などで稼ぐ    ¥20,278   1日あたり ¥676・週 ¥4,732
  固定支出を減らす  ¥20,278   いまの固定支出の 11.2%
```

### 今月の実績と計画

同じ画面で、今月の実績を計画と並べて見られます。

```
今月の実績と計画            25日目 / 31日（80.6%経過）

              計画        実績        差
収入        ¥280,000    ¥250,000   -¥30,000
支出（引落）  ¥180,278    ¥195,000   +¥14,722
返済        ¥120,000     ¥60,000   -¥60,000
残り        ¥-20,278     ¥-5,000   +¥15,278
```

月の進み具合を添えているので、途中の数字も読み違えません。
差の色は「望ましくない向き」を赤にします（収入と返済は少ないと赤、支出は多いと赤）。

**返済目標は最低返済額を含む総額**です。目標が未設定なら、最低返済額の合計を使います。
返済の行は借入の登録から自動で入るので、固定の支出に返済を手で足さないでください。

副業で埋めるか、支出を削るか、その組み合わせかを決める材料になります。

> ここに入れた金額は**計画のための数字**です。実際の収支の記録にはなりません。
> 実績は「収入・支出」画面に記録してください。

```
fixed_items  毎月決まって出入りするお金。type で収入と支出を分ける
```

## ふるさと納税の上限

「ふるさと納税」画面で、**自己負担が 2,000 円で収まる寄付額の上限**を計算します。

上限を決めているのは住民税の**特例分**で、これは所得割額の 20% までしか使えません。
そこから逆算した式が上限です。

```
上限 = 住民税の所得割額 × 20% ÷ (90% − 所得税率 × 1.021) + 2,000 円
```

`1.021` は復興特別所得税（2037 年分まで）。`90%` は 100% から住民税の基本分 10% を引いた残りです。

### 入れる数字

収支の記録から集められるものは自動で埋まります。**空欄なら記録から、入れたらそちらが優先**です。

| 項目 | どこから来るか |
|---|---|
| 事業の売上 | **記録した収入すべて** − 支出の「預かり金の返却」 |
| 経費 | 経費に設定した支出の合計（割合を掛けた額） |
| 青色申告特別控除 | 0 / 10 / 55 / 65 万円から選びます |
| 社会保険料控除 | 支出のカテゴリ「社会保険」の合計 |
| 住民税の所得割額 | 手入力。**入れるとこれが最優先**になります |
| 給与の年収（額面） | 手入力。給与がある人だけ。記録している給与は手取りなので使えません |
| その他の控除 | 手入力（生命保険料・iDeCo・医療費・配偶者と扶養など） |

収入はカテゴリを問わず、すべて事業の売上として数えます。

**給与の年収（額面）を入れたときだけ**、カテゴリ「給与」「賞与」の記録を売上から外します。
記録しているのは手取りなので、額面と足すと同じ収入を二重に数えてしまうためです。
外した額は内訳に出ます。

青色申告特別控除は法律で決まった 3 つの額しかないので、自由入力ではなく選択にしています
（複式簿記＋e-Tax なら 65 万、書面なら 55 万、簡易簿記は 10 万）。
これを使うと事業所得が減るぶん、ふるさと納税の上限も下がります。

**住民税決定通知書（毎年 6 月にもらう紙）の所得割額を入れるのが一番正確です。**
控除の入れ忘れがあっても結果が狂いません。

### 経費の登録

支出を追加するときに「経費にする割合」を選べます。あとから変えることもできて、
ふるさと納税画面の一覧から**カテゴリごとにまとめて**設定できます
（12 ヶ月ぶんの通信費を 1 件ずつ触るのはスマホでは現実的でないため）。

割合は 0 / 25 / 50 / 75 / 100% から選びます。スマホ代のように仕事と私用が混ざるものは、
仕事で使っている割合（家事按分）にします。

**「預かり金の返却」は経費の一覧に出しません。**売上から先に引いてあるので、
経費にも数えると二重になります。

### 寄付済みの額

支出をカテゴリ「ふるさと納税」で記録すると、その年の寄付済み額として数えます。
画面には「今年の残り」が出ます。

### 基礎控除が2つある理由

内訳には基礎控除が**所得税と住民税で別々に**出ます。額が違うためです。

| | 基礎控除 | 何を決めるか |
|---|---|---|
| 所得税 | 合計所得に応じて 58〜95 万円 | 課税所得 → **所得税率**（式の分母） |
| 住民税 | **43 万円**（合計所得 2,400 万円以下は一律） | 課税所得 → **所得割額**（上限のほぼ全部） |

上限を実質的に決めているのは住民税の側です。所得税の側は、式の分母
`90% − 所得税率 × 1.021` に使う税率を選ぶためだけに要ります。

この差のぶん住民税が取りすぎになるので、**調整控除**（多くの場合 2,500 円）で戻します。
内訳では所得割額の行に出ます。

所得税側の 88 / 68 / 63 万円は**令和 7・8 年分だけの上乗せ**です。
令和 9 年分からは 58 万円に戻るので、そのとき `TAX.basicIncome` を直します。

### 精度について

総務省が公表している「全額控除されるふるさと納税額の目安」と突き合わせています
（給与収入・独身、社会保険料を年収の約 14.4% として計算した表）。

| 年収 | 本ツール | 目安表 |
|---|---|---|
| 300 万円 | 28,000 | 28,000 |
| 400 万円 | 42,000 | 42,000 |
| 500 万円 | 61,000 | 61,000 |
| 600 万円 | 78,000 | 77,000 |
| 700 万円 | 109,000 | 108,000 |
| 800 万円 | 131,000 | 129,000 |

高い方でずれるのは、目安表が仮定している社会保険料率との差です。本ツールは記録した実額を使うので、
実際の申告内容に近い数字が出ます。

前提としている数値は `TAX`（`public/index.html`）に集めてあります。令和 7 年（2025 年）の改正を
織り込み済みです（給与所得控除の最低保障 65 万円、所得税の基礎控除の上乗せ）。年が変わったらここだけ直します。

生命保険料控除などは所得税と住民税で上限が違いますが、**同じ額として扱っています。**
住民税の控除を多めに見るぶん上限は控えめに出ます。ふるさと納税は出しすぎが損なので、ずれるなら少ない側が安全です。
画面が勧める「安全な目安」は上限の 9 割です。

**あくまで目安です。** 申し込む前に、寄付先サイトのシミュレーターか住民税決定通知書で確かめてください。

## 計算の前提

- 利息は**日割り**で計算します。**元金 × 年利 ÷ 365日 × 経過日数**。うるう年も 365 日として扱います。
- 利息が付くのは**元金だけ**です。払いきれずに残った未払利息が、それ自身でさらに利息を生むことはありません。
- 返済を記録すると、起算日から返済日までの利息を積んだうえで、返済額を**まず利息に、残りを元金に**充当します。
  返済額が利息に満たなければ元金は1円も減らず、差額が未払利息として積み上がります。
- **追加借入を記録できます。** カードローンのように返済しながらまた借りる形に対応します。
  返済と同じ時間軸に並べ、日付順に再生して残高を出すので、借入を挟んでも計算が合います。
  当初借入額にも加算されるので、返済済みの割合は「借りた総額のうちいくら返したか」になります。
- **返済は入力した順ではなく、日付の順で計算します。** 記録するたびに、起点から
  すべての返済を日付順に再生して残高を組み直すので、あとから過去の返済を足しても
  結果は変わりません。各回の利息と元金の内訳もそのとき振り直されます。
- **起算日より前の返済・追加借入も記録できます。** ただし**残高の計算には入りません**。
  「この日時点でこの残高だった」という申告に既に含まれているため、差し引くと二重に減るからです。
  一覧では「履歴のみ」と表示され、利息と元金の内訳は出ません。
  残高にも反映したい場合は、借入を編集して起算日と元金をその時点の値に直してください。
- 借入を編集すると、その内容が新しい起点になります。
- 支出は**発生ベース**（使った月）と**引落ベース**（口座から出ていく月）の両方で集計します。
  カテゴリ別の内訳は発生ベース、余力の計算とグラフは引落ベースです。
- 「取り置きが必要な額」は、**翌月に引き落とされることが既に確定している支出**の合計です
  （その月までに使った分のみ。翌月に使う予定の分は含みません）。
- **月の返済額は「返済 − その月の追加借入」**で出します。返済した月に同じだけ借りていれば、
  借金は減っていないためです。借りた分は手元に入るお金なので、余力にはその分が加わります。
  借りた方が多い月は、返済額がマイナスで表示されます。
- **余力の計算はすべて引落ベース**です。目標画面の「直近3ヶ月の平均余力」も、
  収入から**口座から出ていった額**を引いて出します（使った額ではありません）。
- **返済は雪だるま式（残高の小さい借入から完済する方式）に固定**しています。
  完済予定日・必要な月額・借入ごとの完済見込みは、すべてこの方式で計算します。
  シミュレーション画面では最低返済のみと雪崩式も並べますが、**参考として出しているだけ**です。
- シミュレーションのスライダーは、毎月の返済額が**合計50万円**に届くまで動かせます。
- シミュレーションは、返済日を今日から1ヶ月刻みに置いて日割りで利息を積みます。
  毎月の返済額が一定で、新たな借入がないことを前提としています。
- 50年（600ヶ月）以内に完済しない場合、または残高が減らない場合は「完済しません」と表示します。
- 実際の約定日・遅延損害金・借入先ごとの端数処理は考慮していません。
  **ここでの数字は計画を立てるための目安**として使ってください。

検証は `npm test`（290件）で、日割り計算そのもの・返済の充当順序・取り消しの復元に加えて、
単一ローンの完済月数を元利均等返済の理論式 `n = -ln(1 - rP/A) / ln(1+r)` と突き合わせています
（日割りでも1年を通せば月複利の理論値と一致します）。
