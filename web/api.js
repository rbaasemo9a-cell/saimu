...
const MUT = {
  addDebt(b) {
    ...
    mem.debts.push(Object.assign({ id: newId(), name }, debtFields(b), { createdAt: nowISO() }));
  },
  ...
  updateDebt(id, b) {
    ...
    const name = strOf(b.name, 60);
    if (!name) throw new Refused('借入先の名前を入力してください');
    Object.assign(d, { name }, debtFields(b));
  },
  ...
  addRepayment(b) {
    ...
    const accrued = d.interestAccrued + accrueOn(d.principal, d.rate, daysBetweenISO(d.accruedAt, date));
    const paidInterest = Math.min(amount, accrued);
    const paidPrincipal = Math.min(amount - paidInterest, d.principal);

    mem.repayments.push({
      id: newId(), debtId: d.id, date, amount,
      interest: paidInterest, principal: paidPrincipal, memo: strOf(b.memo, 60)
    });
    d.principal -= paidPrincipal;
    d.interestAccrued = accrued - paidInterest;
    // 過去の日付で記録しても起算日は戻さない。戻すと同じ期間の利息を二度積む。
    if (date > d.accruedAt) d.accruedAt = date;
  },
  ...
  deleteRepayment(id) {
    ...
    if (d) {
      d.principal += Math.max(0, r.principal);
      d.interestAccrued += Math.max(0, r.interest);
    }
  ...
  loadSample() {
    ...
    const seeds = [
      ...
      const d = { id: newId(), name, principal: prin, interestAccrued: 0,
                  accruedAt: mk(6) + '-27', initial, rate, minPayment,
                  createdAt: mk(10) + '-01' };
      mem.debts.push(d);
      return { d, pay };
    });
